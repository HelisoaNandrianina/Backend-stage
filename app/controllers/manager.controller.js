...
exports.findAll = (req, res) => {
    ...
    User.findAll({
        where: condition, // Applique la condition de recherche 
        order: [['nom', 'ASC']] // Trie les utilisateurs par 'nom' de manière croissante (ASC)
    })
        .then(data => {
            if (data == null) {
                res.json({
                    message: "Il n y a pas de données dans la Base de Donnee"
                })
            } else {
                res.send(data)
            }
